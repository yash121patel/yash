...
var db = new import_better_sqlite3.default("db.sqlite");
db.exec(`
...
if (existingEvents.count === 0) {
  ...
  insertEvent.run(import_crypto.default.randomUUID(), "\u0AB8\u0AB5\u0ABE\u0AB0\u0AA8\u0AC0 \u0AAE\u0A82\u0A97\u0AB3\u0ABE \u0A86\u0AB0\u0AA4\u0AC0", "\u0AE6\u0AEC:\u0AE6\u0AE6 AM", "\u0AA6\u0AB0\u0AB0\u0ACB\u0A9C \u0AB8\u0AB5\u0ABE\u0AB0\u0AC7 \u0AA6\u0ABF\u0AB5\u0ACD\u0AAF \u0AAE\u0AB9\u0ABE \u0A86\u0AB0\u0AA4\u0AC0");
  insertEvent.run(import_crypto.default.randomUUID(), "\u0AAD\u0A95\u0ACD\u0AA4 \u0AAD\u0ACB\u0A9C\u0AA8 \u0AAA\u0ACD\u0AB0\u0AB8\u0ABE\u0AA6\u0AB6\u0ABE\u0AB3\u0ABE", "\u0AE7\u0AE7:\u0AE9\u0AE6 AM - \u0AE6\u0AE8:\u0AE6\u0AE6 PM", "\u0AB6\u0ACD\u0AB0\u0AC0 \u0A9A\u0AC7\u0AB9\u0AB0 \u0AAA\u0ACD\u0AB0\u0AB8\u0ABE\u0AA6 \u0AB5\u0ABF\u0AA4\u0AB0\u0AA3 \u0AAD\u0ACB\u0A9C\u0AA8\u0ABE\u0AB2\u0AAF");
  insertEvent.run(import_crypto.default.randomUUID(), "\u0AB8\u0ABE\u0A82\u0A9C\u0AA8\u0AC0 \u0AB8\u0A82\u0AA7\u0ACD\u0AAF\u0ABE \u0A86\u0AB0\u0AA4\u0AC0", "\u0AE6\u0AED:\u0AE6\u0AE6 PM", "\u0AB8\u0ABE\u0A82\u0A9C\u0AA8\u0ABE \u0AB8\u0AAE\u0AAF\u0AA8\u0AC0 \u0AA7\u0AC2\u0AAA \u0A86\u0AB0\u0AA4\u0AC0");
  insertEvent.run(import_crypto.default.randomUUID(), "\u0AB0\u0ABE\u0AA4\u0ACD\u0AB0\u0AC0 \u0AAD\u0A9C\u0AA8 \u0AB8\u0AA4\u0ACD\u0AB8\u0A82\u0A97 \u0AAE\u0A82\u0AA1\u0AB3", "\u0AE6\u0AEF:\u0AE6\u0AE6 PM", "\u0AB6\u0AA8\u0ABF\u0AB5\u0ABE\u0AB0 \u0A85\u0AA8\u0AC7 \u0AB0\u0AB5\u0ABF\u0AB5\u0ABE\u0AB0\u0AC7 \u0A96\u0ABE\u0AB8 \u0A86\u0AAF\u0ACB\u0A9C\u0AA8");
}
var generateToken = () => {
...
app.post("/api/events/daily", (req, res) => {
  try {
    ...
    const eventId = id || import_crypto.default.randomUUID();
    db.prepare(`
...
app.get("/api/backups", (req, res) => {
  try {
    const files = import_fs.default.readdirSync(BACKUPS_DIR).filter((f) => f.endsWith(".sqlite")).map((filename) => {
      const stats = import_fs.default.statSync(import_path.default.join(BACKUPS_DIR, filename));
      return {
        filename,
        size: stats.size,
...
app.get("/api/backup/:filename/devotees", (req, res) => {
  try {
    ...
    const backupDb = new import_better_sqlite3.default(backupPath, { readonly: true });
    try {
...
app.post("/api/restore", (req, res) => {
  try {
    ...
    db.close();
    import_fs.default.copyFileSync(backupPath, "db.sqlite");
    db = new import_better_sqlite3.default("db.sqlite");
    io.emit("update_data");
    io.emit("events_update");
    io.emit("live_tv_update");
    io.emit("registration_schedule_update");
...
async function startServer() {
  const distPath = import_path.default.join(process.cwd(), "../client/dist");
